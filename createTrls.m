...
elseif ~useSameTrls && jointTrls  %joined trials
    %move step sizes - possible step sizes; if larger, e.g. double, means bigger steps
    ...
    if ~strcmp(dat(1:4),'trap') % regular shapes (circ, square, rect)
        ...
        %trapz - left right don't make too big steps - since height is more than double the width
...
%dataPtsTest - for initiating cluster centres, better to not select from the trials
